...
export class InitialSchema1605442894974 implements MigrationInterface {
  name = 'InitialSchema1605442894974'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('CREATE TABLE `company_register_configs` (`id` int NOT NULL AUTO_INCREMENT, `store_nationality` tinyint NOT NULL DEFAULT 0, `store_address` tinyint NOT NULL DEFAULT 0, `store_phone_number` tinyint NOT NULL DEFAULT 0, `store_birthplace` tinyint NOT NULL DEFAULT 0, `store_birth_date` tinyint NOT NULL DEFAULT 0, `store_mother_name` tinyint NOT NULL DEFAULT 0, `store_company` tinyint NOT NULL DEFAULT 0, `register_guest_card` tinyint NOT NULL DEFAULT 1, `track_actual_exit` tinyint NOT NULL DEFAULT 1, PRIMARY KEY (`id`)) ENGINE=InnoDB')
    ...
    await queryRunner.query("CREATE TABLE `guests` (`id` int NOT NULL AUTO_INCREMENT, `nationality` varchar(255) NULL, `phone_number` varchar(255) NULL, `birthplace` varchar(255) NULL, `birth_date` varchar(255) NULL, `mother_name` varchar(255) NULL, `image_url` varchar(255) NULL, `identifier_card_type` enum ('identity_card', 'student_card', 'driving_license') NOT NULL, `identifier_card_number` varchar(255) NULL, `signature_image_url` varchar(255) NULL, `participation_status` enum ('invited', 'confirmed', 'canceled', 'participated', 'missed') NOT NULL, `actual_entry` timestamp NULL, `actual_exit` timestamp NULL, `created_at` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `user_id` int NOT NULL, `address_id` int NULL, `receptionist_id` int NULL, `guest_card_id` int NULL, `company_id` int NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB")
    await queryRunner.query("CREATE TABLE `visits` (`id` int NOT NULL AUTO_INCREMENT, `planned_entry` timestamp NOT NULL, `purpose` enum ('meeting', 'interview') NOT NULL, `room` varchar(255) NOT NULL, `expiration` timestamp NULL, `created_at` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `business_host_id` int NOT NULL, `company_id` int NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB")
    await queryRunner.query('CREATE TABLE `consent_form_versions` (`id` int NOT NULL AUTO_INCREMENT, `content` text NOT NULL, `version_number` int NOT NULL, `created_at` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `consent_form_id` int NOT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB')
    ...
    await queryRunner.query('CREATE TABLE `companies` (`id` int NOT NULL AUTO_INCREMENT, `name` varchar(255) NOT NULL, `registration_number` varchar(255) NOT NULL, `created_at` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `deleted_at` timestamp(6) NULL DEFAULT NULL, `address_id` int NULL, `office_building_id` int NULL, `register_config_id` int NULL, `admin_id` int NULL, UNIQUE INDEX `IDX_3dacbb3eb4f095e29372ff8e13` (`name`), UNIQUE INDEX `IDX_21bc5437316daf4448d2db3380` (`registration_number`), UNIQUE INDEX `REL_8997ad7f2d7c7d677903a808ee` (`register_config_id`), UNIQUE INDEX `REL_4b6ba77c89a9ef7461fa133738` (`admin_id`), PRIMARY KEY (`id`)) ENGINE=InnoDB')
    ...
    await queryRunner.query('CREATE TABLE `users` (`id` int NOT NULL AUTO_INCREMENT, `first_name` varchar(255) NOT NULL, `last_name` varchar(255) NOT NULL, `email` varchar(255) NOT NULL, `password` varchar(255) NOT NULL, `password_token` varchar(255) NULL, `created_at` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `deleted_at` timestamp(6) NULL DEFAULT NULL, `role_id` int NULL, `company_id` int NULL, UNIQUE INDEX `IDX_97672ac88f789774dd47f7c8be` (`email`), PRIMARY KEY (`id`)) ENGINE=InnoDB')
    await queryRunner.query('CREATE TABLE `office_buildings` (`id` int NOT NULL AUTO_INCREMENT, `created_at` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `deleted_at` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6), `address_id` int NULL, `admin_id` int NOT NULL, UNIQUE INDEX `REL_fb68ae56d91d47561add00bba9` (`admin_id`), PRIMARY KEY (`id`)) ENGINE=InnoDB')
    ...
    await queryRunner.query('ALTER TABLE `guests` ADD CONSTRAINT `FK_23176a4bf8976cdcafaff760921` FOREIGN KEY (`receptionist_id`) REFERENCES `users`(`id`) ON DELETE SET NULL ON UPDATE NO ACTION')
    ...
    await queryRunner.query('ALTER TABLE `guests` ADD CONSTRAINT `FK_59082e09038a9ba5139c38ff685` FOREIGN KEY (`company_id`) REFERENCES `companies`(`id`) ON DELETE SET NULL ON UPDATE NO ACTION')
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('ALTER TABLE `guests` DROP FOREIGN KEY `FK_59082e09038a9ba5139c38ff685`')
